...
module.exports = app => {
  ...
  userSchema.statics.generateHashPassword = async function generateHashPassword(
    pass,
    repeated
  ) {
    const dataPass = {}
        ...
        if (passValidation.validate(pass, { list: true })) {
          await isPasswordBlacklisted(pass).then(async isBlacklisted => {
            if (isBlacklisted) {
              ...
              const hash = await bcrypt.hash(pass, scriptParameters)
  ...
  userSchema.statics.resetUserPassword = async function resetUserPassword(
    req,
    res
  ) {
    ...
    if (data.newPassword && data.repeatedNewPass) {
      await this.generateHashPassword(data.newPassword, data.repeatedNewPass)
        .then(async pass => {
          const update = {
            hash: pass.hash,
  ...
  userSchema.statics.forgotUserPassword = async function forgotUserPassword(
    req,
    res
  ) {
    try {
      ...
      if (validator.isEmail(data.email)) {
        ...
        await this.sendResetPasswordEmail(req, res, token)
          .then(() => {
            return res.status(200).json({
              token,
  ...
  userSchema.statics.redirectToResetPassword = async function redirectToResetPassword(
    req,
    res
  ) {
    res.statusCode = 302
    res.setHeader(
      'Location',
      app.config.UIPath + '/resetpassword?rt=' + req.token
    )
    res.end()
  }

  userSchema.statics.sendResetPasswordEmail = async function sendResetPasswordEmail(
    req,
    res,
    token
  ) {
  ...
  userSchema.statics.privacyPolicyConfirmation = async function privacyPolicyConfirmation(
    req,
    res
  ) {
    const user = req.user
  ...
  userSchema.statics.termsAndConditionsConfirm = async function termsAndConditionsConfirm(
    req,
    res
  ) {
    const user = req.user
  ...
  userSchema.statics.cookiesPolicyConfirmation = async function cookiesPolicyConfirmation(
    req,
    res
  ) {
    const user = req.user
  ...
  userSchema.statics.redirectToPrivacyPolicy = async function redirectToPrivacyPolicy(
    req,
    res
  ) {
    res.statusCode = 302
    res.setHeader(
      'Location',
      app.config.UIPath + '/privacypolicy?rt=' + req.token
    )
    res.end()
  }

  userSchema.statics.sendRegistrationEmail = async function sendRegistrationEmail(
    req,
    res,
    token
  ) {
  ...
  userSchema.statics.registerUserAccount = async function registerUserAccount(
    req,
    res
  ) {
    const data = req.body
    const pass = await this.generateHashPassword(
      data.password,
      data.repeatedPass
    )
  ...
  userSchema.statics.logoutAllUserAccount = async function logoutAllUserAccount(
    req,
    res
  ) {
    try {
  ...
  userSchema.statics.logoutUserAccount = async function logoutUserAccount(
    req,
    res
  ) {
    try {
  ...
  userSchema.statics.loginUserAccount = async function loginUserAccount(
    req,
    res
  ) {
    const data = req.body
    if (
      ...
      passValidation.validate(data.password)
    ) {
      try {
  ...
  userSchema.statics.updateUserAccount = async function updateUserAccount(
    req,
    res
  ) {
    const userReq = req.user
    ...
    if (data.newPassword && data.repeatedNewPass) {
      await this.generateHashPassword(data.newPassword, data.repeatedNewPass)
        .then(async pass => {
          ...
          await user.update({ runValidators: true }, null)
          return res.status(200).json({
            user
          })
        })
  ...
  userSchema.methods.generateAuthToken = async function generateAuthToken(
    ...
    let token = null
    if (isReset) {
      token = jwt.sign({ _id: user._id }, app.config.JWT_KEY, {
        expiresIn: '10h'
      })
    } else {
      token = jwt.sign({ _id: user._id }, app.config.JWT_KEY, {
